...
import { Search, ChevronLeft, ChevronRight, Inbox, AlertCircle, BedDouble } from "lucide-react";
...
export default function GetReservations() {
    ...
    useEffect(() => {
        const getReservations = async () => {
            try {
                const request = await axios.get("http://localhost:3000/api/reservation", {
    ...
    const filtered = useMemo(() => {
        return reservations.filter((r) => {
            ...
            const fullName = `${r.customer?.name ?? ""} ${r.customer?.surname ?? ""}`.toLowerCase();
            const matchesQuery =
                q === "" ||
                fullName.includes(q) ||
        ...
        <div className="p-8 bg-slate-50 min-h-screen">
            <div className="max-w-6xl mx-auto">
                <div className="flex items-end justify-between mb-6">
                    <div>
                        <h1 className="text-2xl font-semibold text-slate-900 tracking-tight">Reservations</h1>
                        <p className="text-sm text-slate-500 mt-1">
                            {loading ? "Loading" : `${filtered.length} Reservations are listing...`}
                        </p>
                    </div>
                </div>
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div className="flex items-center gap-1 overflow-x-auto">
                            {STATUS_TABS.map((tab) => (
                                <button
                                    ...
                                    className={` cursor-pointer px-3 py-1.5 rounded-md text-sm font-medium whitespace-nowrap transition-colors ${statusFilter === tab
                    ...
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center gap-2 py-16 text-center">
                            ...
                            <p className="text-sm font-medium text-slate-700">No Reservations</p>
                        ...
                        <>
                                        ...
                                        {paginated.map((r) => {
                                            ...
                                            return (
                                                <tr key={r._id} className="hover:bg-slate-50/80 transition-colors">
                                                    <td className="px-5 py-3.5">
                                                        <div className="flex items-center gap-3">
                                                            <div className="w-8 h-8 rounded-full bg-blue-100 text-blue-700 text-xs font-semibold flex items-center justify-center shrink-0">
                                                                {initials(r.customer?.name, r.customer?.surname)}
                                                            </div>
                                                            <div>
                                                    ...
                                                    <td className="px-5 py-3.5">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <div>
                                                                ...
                                                                <div className="text-xs text-slate-400">
                                                                    {r.rooms?.roomType ? ROOM_TYPE_LABELS[r.rooms.roomType] ?? r.rooms.roomType : ""}
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="px-5 py-3.5 text-center text-slate-600">{r.guestCount}</td>
                                                    <td className="px-5 py-3.5 text-right">
                                                        <div className="font-medium text-slate-900">{formatCurrency(total)}</div>
                                                        {isPartiallyPaid && (
                                                            <div className="text-xs text-amber-600">{formatCurrency(paid)}Paid</div>
                                                        )}
                                                    </td>
                                                    ...
                                                    <td className="px-5 py-3.5">
                                                        <span
                                                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium whitespace-nowrap ${STATUS_STYLES[r.status] ?? "bg-slate-100 text-slate-600"
                                                                }`}
                                                        >
                            ...
                            <div className="flex items-center justify-between px-5 py-4 border-t border-slate-200">
                                <p className="text-xs text-slate-500">
                                    Page <span className="font-medium text-slate-700">{currentPage}</span> / {totalPages}
                                </p>
